...
<body>
    ...
    <div id="container">
        ...
        <div id="boxAnnexe">
            <h1><?php echo htmlentities($dataAPI['price']) . "€" ?></h1>
            <img id="pictureProcduct" src="<?php echo htmlentities($dataAPI['image']) ?>">
            <hr id="lineSeparator">

            <?php if ($dataAPI['alreadySaved'] == 1) : ?>

                <p><?php echo htmlentities($dataAPI['alertPrice']) ?></p>

                <form id="formPrice" action="changeAlertPrice.php" method="get">
                    <input id="inputPrice" placeholder="New price ?" name="newPrice">
                    <input value="<?php echo htmlentities($dataAPI['asin']); ?>" name="asin" style="display: none;">
        ...
        <div id="boxChart" style="display: flex;">
            ...
            if ($dataAPI['alreadySaved'] != 1) {
                echo "<form action='addProduct.php?asin={$asin}&direction=productPage' id='formAddProduct' method='POST'>
    ...
    <script>
        if (<?php echo "'{$dataAPI['alreadySaved']}'" ?> == 1) {
            ...
            const chart = new Chart(ctx, {
                type: 'line',
                data: {
                    labels: <?php if ($dataAPI['alreadySaved']) {
                                echo json_encode($newArrayDate);
                            } else {
                                echo "[]";
                    ...
                    datasets: [{
                        ...
                        data: <?php if ($dataAPI['alreadySaved']) {
                                    echo json_encode($prices);
                                } else {
                                    echo "[]";
                                }  ?>, //[0, 10, 5, 2, 20, 65, 45]
